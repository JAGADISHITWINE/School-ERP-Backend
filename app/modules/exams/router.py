...
from app.modules.exams.schema import ExamCreate, ExamUpdate, ExamSubjectCreate, MarksUploadRequest, ExamOut, MarkOut
...
async def advance_workflow(exam_id: str, action: str, db: DB):
    """action = 'submit' or 'lock'"""
    exam = await service.advance_workflow(db, exam_id, action)
...
async def upload_marks(payload: MarksUploadRequest, db: DB):
    count = await service.upload_marks(db, payload)
    return ok(data={"records_saved": count}, message="Marks uploaded")


@router.get("/marks/{exam_subject_id}", response_model=dict)
